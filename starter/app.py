...
def generate_app(get_processed_inputs, inpaint):
    ...
    def reset_points(*args):
        ...
        input_points.clear()
        input_image = None
        # Re-enable interactive mode so user can upload a new image
    ...
    def preprocess(input_img):
        ...
        # Disable interactive mode after upload so select events (clicks) work
        return gr.update(value=input_img.resize((IMG_SIZE, IMG_SIZE)), interactive=False)
    ...
    with gr.Blocks() as demo:
        ...
        display_img.upload(preprocess, inputs=[display_img], outputs=[display_img])
        ...
        with gr.Row():
            ...
            # Reset points and restore interactive mode for new uploads
            reset_points_b.click(reset_points, outputs=[display_img])
